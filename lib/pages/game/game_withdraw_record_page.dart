...
class GameWithdrawRecordPage extends StatefulWidget {
  const GameWithdrawRecordPage({super.key});

  @override
  State<GameWithdrawRecordPage> createState() => _GameWithdrawRecordPageState();
}

class _GameWithdrawRecordPageState extends State<GameWithdrawRecordPage> {
  ...
  Future<void> _loadMore({bool reload = false}) async {
    ...
    try {
      final next = await GameApi.getWithdrawRecords(
        ...
        : RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) => index == _records.length
                  ? _footer()
                  : _WithdrawRecordTile(record: _records[index]),
            ),
          ),
  );
...
class _WithdrawRecordTile extends StatelessWidget {
  const _WithdrawRecordTile({required this.record});
      ...
      children: <Widget>[
        Expanded(
          child: Column(
            ...
            children: <Widget>[
              const Text('提现', style: TextStyle(fontSize: 14)),
              const Spacer(),
              Text(
...
Color _statusColor(int status) => status == 3
    ? Colors.lightGreen
    : status == 1 || status == 2
    ? Colors.blueAccent
    : Colors.redAccent;
